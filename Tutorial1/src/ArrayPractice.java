...
public class ArrayPractice{
  ...
  public static int[] merge(int[] a, int[] b){
    ...
    while (index < result.length){
      if (index_a == (a.length)){ // put at the beginning to avoid program from checking an index in an array that isn't there
      ...
      else { //if (a[index_a] >= b[index_b]) : what it does
        result[index] = b[index_b];
        index_b += 1;
        index += 1;
      }
